...
describe("script runtime contracts", () => {
  ...
  it("runs Lua with injected Vortex Web APIs", async () => {
    ...
    await adapter.run([
      ...
      "print(vweb.camera.setSubject('tester').ok)",
      "print(vweb.camera.getSubject().name)",
      "print(vweb.camera.clearSubject().ok)",
      ...
      api: {
        ...
        "camera.setSubject": () => ({ ok: true, id: "1", name: "tester", local: true }),
        "camera.setTarget": () => ({ ok: true, id: "1", name: "tester", local: true }),
        "camera.clearSubject": () => ({ ok: true }),
        "camera.getSubject": () => ({ id: "1", name: "tester", local: true }),
