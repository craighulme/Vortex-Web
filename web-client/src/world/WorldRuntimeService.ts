...
import type { WorldDynamicAdapter, WorldDynamicObjectService } from "./WorldDynamicObjectService";
...
type WorldRuntimeConfig = {
  ...
  dynamicObjects: WorldDynamicObjectService;
...
export class WorldRuntimeService {
  configure(config: WorldRuntimeConfig): WorldRuntimeHandles {
    ...
    const dynamicObjects = config.dynamicObjects.configure({
      ...
      parts: partService,
      bufferGeometryUtils: config.THREE.BufferGeometryUtils,
      shadowsActive: () => config.shadows.active()
    });
